...
// Funzione helper per generare e salvare i token (così non ripetiamo codice)
function generateUserTokens($email, $nome, $mysqli) {
    // 1. JWT (Access Token)
    $issuedAt = time();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && 
    ...
    if(isset($data->email) && isset($data->password)){
        ...
        if ($row && password_verify($data->password, $row['password'])) {
            $tokens = generateUserTokens($row['email'], $row['nome'], $mysqli);
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    if (!empty($email) && !empty($password)) {
        ...
        if ($row && password_verify($password, $row['password'])) {
            // SALVA IL REFRESH TOKEN ANCHE QUI!
            generateUserTokens($row['email'], $row['nome'], $mysqli);
